(() => {
  ...
  const STAT_POINT_PER_STAT = 32;
  const BATTLE_STAGE_STATS = [["attack","공격"],["defense","방어"],["spAttack","특공"],["spDefense","특방"],["speed","스피드"],["accuracy","명중률"],["evasion","회피율"]];
  const BATTLE_STAGE_LABEL = Object.fromEntries(BATTLE_STAGE_STATS);
  ...
  let MOVES = [
    ...
    { id:"roar", name:"울부짖기", type:"노말", category:"status", accuracy:100, alwaysHit:true, priority:-6, sound:true, forceSwitch:true, pp:20, description:"상대를 강제로 교체시킨다. 교체되어 나온 포켓몬은 설치된 장판의 영향을 받는다." },
    { id:"stealthrock", name:"스텔스록", type:"바위", category:"status", accuracy:100, sideCondition:"stealthrock", pp:20, description:"상대 필드에 스텔스록을 설치한다. 교체되어 나온 포켓몬은 바위타입 상성에 따라 최대 HP의 1/32~1/2 피해를 받는다." },
  ...
  const FULL_DEX_CACHE_KEY = "nexus-full-dex-cache-v3";
  const LEGACY_FULL_DEX_CACHE_KEYS = ["nexus-full-dex-cache-v2","nexus-full-dex-cache-v1"];
  ...
  const BOOST_KEY = { atk:"attack",def:"defense",spa:"spAttack",spd:"spDefense",spe:"speed",accuracy:"accuracy",evasion:"evasion" };
  const BOOST_LABEL = { atk:"공격",def:"방어",spa:"특수공격",spd:"특수방어",spe:"스피드",accuracy:"명중률",evasion:"회피율" };
  const MOVE_ID_ALIASES = Object.create(null);
  const ABILITY_ID_ALIASES = Object.create(null);
  ...
  function fractionPercent(value) {
    return Array.isArray(value)&&Number(value[1]) ? Math.round(Math.abs(Number(value[0])/Number(value[1]))*100) : 0;
  }
  function boostDescription(boosts,subject,chance=100) {
    const changes=Object.entries(boosts||{}).filter(([,amount])=>Number(amount)!==0).map(([key,amount])=>`${BOOST_LABEL[key]||key}을 ${Math.abs(Number(amount))}랭크 ${Number(amount)>0?"올린다":"낮춘다"}`);
    if(!changes.length)return "";
    return `${chance<100?`${chance}% 확률로 `:""}${subject}의 ${changes.join(", ")}`;
  }
  function koreanMoveDescription(data) {
    const parts=[],type=EN_TYPE_TO_KO[data.type]||"노말",category=String(data.category||"Status").toLowerCase(),categoryLabel=category==="physical"?"물리":category==="special"?"특수":"변화";
    if(data.ohko)parts.push("명중하면 상대를 한 번에 쓰러뜨린다");
    ...
    else if(data.basePowerCallback)parts.push(`조건에 따라 위력이 달라지는 ${type}타입 ${categoryLabel} 공격이다`);
    else if(Number(data.basePower)>0)parts.push(`위력 ${Number(data.basePower)}의 ${type}타입 ${categoryLabel} 공격이다`);
    else if(category==="status")parts.push(`${type}타입 변화기술이다`);
    else parts.push(`특수한 방식으로 데미지를 주는 ${type}타입 ${categoryLabel} 공격이다`);
    if(data.accuracy===true)parts.push("명중률과 회피율에 관계없이 명중한다");
    if(Number(data.priority)>0)parts.push(`우선도 +${Number(data.priority)}로 먼저 사용하기 쉽다`);
    if(Number(data.priority)<0)parts.push(`우선도 ${Number(data.priority)}로 늦게 사용한다`);
    if(data.status)parts.push(`상대를 ${STATUS_LABEL[STATUS_CODE[data.status]||data.status]||"상태이상"} 상태로 만든다`);
    const directBoost=boostDescription(data.boosts,data.target==="self"?"자신":"상대");if(directBoost)parts.push(directBoost);
    const selfBoost=boostDescription(data.self?.boosts,"자신");if(selfBoost)parts.push(selfBoost);
    const secondaries=[data.secondary,...(Array.isArray(data.secondaries)?data.secondaries:[])].filter(Boolean);
    for(const secondary of secondaries){const chance=Number(secondary.chance)||100;if(secondary.status)parts.push(`${chance}% 확률로 상대를 ${STATUS_LABEL[STATUS_CODE[secondary.status]||secondary.status]||"상태이상"} 상태로 만든다`);if(secondary.volatileStatus==="flinch")parts.push(`${chance}% 확률로 상대를 풀죽게 한다`);const targetBoost=boostDescription(secondary.boosts,"상대",chance);if(targetBoost)parts.push(targetBoost);const secondarySelfBoost=boostDescription(secondary.self?.boosts,"자신",chance);if(secondarySelfBoost)parts.push(secondarySelfBoost);}
    const drain=fractionPercent(data.drain);if(drain)parts.push(`준 데미지의 ${drain}%만큼 HP를 회복한다`);
    const recoil=fractionPercent(data.recoil);if(recoil)parts.push(`준 데미지의 ${recoil}%를 반동으로 받는다`);
    const heal=fractionPercent(data.heal);if(heal)parts.push(`최대 HP의 ${heal}%를 회복한다`);
    if(data.stallingMove)parts.push("그 턴에 받는 대부분의 공격을 막는다");
    if(Number(data.critRatio)>1)parts.push("급소에 맞을 확률이 높다");
    if(Array.isArray(data.multihit))parts.push(`${data.multihit[0]}~${data.multihit[1]}회 연속으로 공격한다`);
    else if(Number(data.multihit)>1)parts.push(`${Number(data.multihit)}회 연속으로 공격한다`);
    if(data.selfdestruct)parts.push("사용한 뒤 자신은 쓰러진다");
    if(data.forceSwitch)parts.push("상대를 강제로 교체시킨다");
    if(dexId(data.sideCondition)==="stealthrock")parts.push("상대 필드에 스텔스록을 설치하며, 교체된 포켓몬에게 바위타입 상성에 따른 피해를 준다");
    if(data.basePowerCallback||data.condition||data.weather||data.terrain||(data.sideCondition&&dexId(data.sideCondition)!=="stealthrock")||data.slotCondition||data.selfSwitch||data.callsMove||data.twoTurnMove)parts.push("전투 조건에 따라 추가 효과가 달라질 수 있다");
    return `${[...new Set(parts)].join(". ")}.`;
  }
  function displayNameKey(value="") { return String(value).normalize("NFKC").toLowerCase().replace(/[\s·ㆍ\-_'’]/g,""); }
  function rebuildDexIndexes() {
    ...
    for(const [alias,id] of Object.entries(MOVE_ID_ALIASES))if(MOVE_BY_ID[id])MOVE_BY_ID[alias]=MOVE_BY_ID[id];
    for(const [alias,id] of Object.entries(ABILITY_ID_ALIASES))if(ABILITY_BY_ID[id])ABILITY_BY_ID[alias]=ABILITY_BY_ID[id];
  }
  function mergeFullDex(extraMoves=[],extraAbilities=[]) {
    const moveAlias=Object.fromEntries(MOVES.map(move=>[dexId(move.id),move.id])),moveNum=Object.fromEntries(MOVES.filter(move=>move.num).map(move=>[move.num,move.id])),moveName=Object.fromEntries(MOVES.map(move=>[displayNameKey(move.name),move.id]));
    const abilityAlias=Object.fromEntries(ABILITIES.map(ability=>[dexId(ability.id),ability.id])),abilityNum=Object.fromEntries(ABILITIES.filter(ability=>ability.num).map(ability=>[ability.num,ability.id])),abilityName=Object.fromEntries(ABILITIES.map(ability=>[displayNameKey(ability.name),ability.id]));
    for(const incoming of extraMoves){const incomingKey=dexId(incoming.id),alias=moveAlias[incomingKey]||moveNum[incoming.num]||moveName[displayNameKey(incoming.name)],existing=alias&&(MOVE_BY_ID[alias]||MOVES.find(move=>move.id===alias));if(existing){const canonicalId=existing.id,coverage=existing.engineCoverage==="verified"?"verified":incoming.engineCoverage;Object.assign(existing,incoming,{id:canonicalId,engineCoverage:coverage});MOVE_ID_ALIASES[incomingKey]=canonicalId;moveAlias[incomingKey]=canonicalId;if(incoming.num)moveNum[incoming.num]=canonicalId;moveName[displayNameKey(incoming.name)]=canonicalId;}else{MOVES.push(incoming);moveAlias[incomingKey]=incoming.id;MOVE_ID_ALIASES[incomingKey]=incoming.id;if(incoming.num)moveNum[incoming.num]=incoming.id;moveName[displayNameKey(incoming.name)]=incoming.id;}}
    for(const incoming of extraAbilities){const incomingKey=dexId(incoming.id),alias=abilityAlias[incomingKey]||abilityNum[incoming.num]||abilityName[displayNameKey(incoming.name)],existing=alias&&(ABILITY_BY_ID[alias]||ABILITIES.find(ability=>ability.id===alias));if(existing){const canonicalId=existing.id,coverage=existing.engineCoverage==="verified"?"verified":incoming.engineCoverage,description=incoming.descriptionSource==="fallback"&&existing.description?existing.description:incoming.description;Object.assign(existing,incoming,{id:canonicalId,description,engineCoverage:coverage});ABILITY_ID_ALIASES[incomingKey]=canonicalId;abilityAlias[incomingKey]=canonicalId;if(incoming.num)abilityNum[incoming.num]=canonicalId;abilityName[displayNameKey(incoming.name)]=canonicalId;}else{ABILITIES.push(incoming);abilityAlias[incomingKey]=incoming.id;ABILITY_ID_ALIASES[incomingKey]=incoming.id;if(incoming.num)abilityNum[incoming.num]=incoming.id;abilityName[displayNameKey(incoming.name)]=incoming.id;}}
  ...
  function parseFandomTable(html,kind) {
    ...
    for (const row of doc.querySelectorAll("table tr")) {
      ...
      const name=cells[1].textContent.trim().replace(/\[[^\]]*\]/g,"").replace(/\s+/g," ");
      const description=kind==="ability"?(cells.slice(2).map(cell=>cell.textContent.trim()).find(text=>text.length>8)||"").replace(/\s+/g," "):"";
      if (num&&name&&!result.has(num)) result.set(num,{name,description});
    }
    return result;
  }
  ...
  function convertShowdownMove(key,data,korean) {
    ...
    const sideCondition=dexId(data.sideCondition),unsupported=!!(data.basePowerCallback||data.condition||data.weather||data.terrain||(sideCondition&&sideCondition!=="stealthrock")||data.slotCondition||data.selfSwitch||data.callsMove||data.twoTurnMove);
    const move={id:dexId(key),num:data.num,englishName:data.name,name:korean?.name||data.name,type:EN_TYPE_TO_KO[data.type]||"노말",category:String(data.category||"Status").toLowerCase(),power:Number(data.basePower)||0,variablePower:!!data.basePowerCallback,accuracy:data.accuracy===true?100:(Number(data.accuracy)||100),alwaysHit:data.accuracy===true,pp:Number(data.pp)||1,priority:Number(data.priority)||0,contact:!!data.flags?.contact,sound:!!data.flags?.sound,punch:!!data.flags?.punch,bite:!!data.flags?.bite,slicing:!!data.flags?.slicing,pulse:!!data.flags?.pulse,bullet:!!data.flags?.bullet,wind:!!data.flags?.wind,recoil:Array.isArray(data.recoil)?Math.abs(data.recoil[0]/data.recoil[1]):0,drain:Array.isArray(data.drain)&&data.drain[0]>0?data.drain[0]/data.drain[1]:0,highCrit:Number(data.critRatio)>1,multihit:data.multihit||null,ohko:!!data.ohko,fixedDamage:data.damage||null,selfDestruct:!!data.selfdestruct,forceSwitch:!!data.forceSwitch,sideCondition:sideCondition||"",description:koreanMoveDescription(data),sourceDescription:data.shortDesc||"",isNonstandard:data.isNonstandard||"",engineCoverage:unsupported?"partial":"auto"};
  ...
  function convertShowdownAbility(key,data,korean) {
    const koreanDescription=String(korean?.description||"").replace(/\s+/g," ").trim();
    return {id:dexId(key),num:data.num,englishName:data.name,name:korean?.name||data.name,description:koreanDescription||"전투 중 이 포켓몬만의 고유한 특성 효과를 발휘한다.",sourceDescription:data.shortDesc||"",descriptionSource:koreanDescription?"ko-wiki":"fallback",isNonstandard:data.isNonstandard||"",engineCoverage:"reference"};
  }
  function isSelectableMove(key,data) {
    if(!data||Number(data.num)<=0||data.isZ||data.isMax)return false;
    if(["CAP","Custom"].includes(String(data.isNonstandard||"")))return false;
    return !/^(hiddenpower|return|frustration)\d+$/i.test(key);
  }
  function isSelectableAbility(key,data) {
    return !!data&&Number(data.num)>0&&!['CAP','Custom'].includes(String(data.isNonstandard||''))&&dexId(key)!=="noability";
  }
  ...
  function loadCachedFullDex() {
    try { const cache=JSON.parse(localStorage.getItem(FULL_DEX_CACHE_KEY));if(cache?.version===3&&Array.isArray(cache.moves)&&Array.isArray(cache.abilities)){mergeFullDex(cache.moves,cache.abilities);fullDexState={status:"ready",moves:MOVES.length,abilities:ABILITIES.length,source:"저장된 전체 데이터"};return true;} } catch {}
    return false;
  }
  async function loadFullDex() {
    ...
    try {
      ...
      const convertedMoves=Object.entries(moveRaw).filter(([key,data])=>isSelectableMove(key,data)).map(([key,data])=>convertShowdownMove(key,data,koMoves.get(Number(data.num))));
      const convertedAbilities=Object.entries(abilityRaw).filter(([key,data])=>isSelectableAbility(key,data)).map(([key,data])=>convertShowdownAbility(key,data,koAbilities.get(Number(data.num))));
      mergeFullDex(convertedMoves,convertedAbilities);
      try{LEGACY_FULL_DEX_CACHE_KEYS.forEach(key=>localStorage.removeItem(key));localStorage.setItem(FULL_DEX_CACHE_KEY,JSON.stringify({version:3,moves:convertedMoves,abilities:convertedAbilities}));}catch{}
      fullDexState={status:"ready",moves:MOVES.length,abilities:ABILITIES.length,source:"Pokémon Showdown + 한국 포켓몬 위키"};
      state=normalizeState(state);saveState("dex-normalize",{silent:true});
  ...
  function canonicalMoveId(value) { const raw=String(value||"");return MOVE_BY_ID[raw]?.id||MOVE_BY_ID[dexId(raw)]?.id||raw; }
  function canonicalAbilityId(value) { const raw=String(value||"");return ABILITY_BY_ID[raw]?.id||ABILITY_BY_ID[dexId(raw)]?.id||raw; }
  function normalizeStatPoints(raw={},trainingSystem="") {
    const source=Object.fromEntries(STATS.map(([key])=>[key,Math.max(0,Number(raw?.[key])||0)])),legacy=trainingSystem!=="sp66"&&Object.values(source).some(value=>value>STAT_POINT_PER_STAT);
    const result=Object.fromEntries(STATS.map(([key])=>[key,clamp(legacy?Math.round(source[key]*STAT_POINT_PER_STAT/252):source[key],0,STAT_POINT_PER_STAT)]));
    let total=Object.values(result).reduce((sum,value)=>sum+value,0);
    while(total>STAT_POINT_TOTAL){const key=STATS.map(([stat])=>stat).sort((a,b)=>result[b]-result[a])[0];result[key]-=1;total-=1;}
    return result;
  }
  function defaultBuild(species) { return { speciesId:species.id, abilityId:species.allowedAbilityIds[0], moveIds:species.allowedMoveIds.slice(0,4), evs:Object.fromEntries(STATS.map(([key])=>[key,0])), trainingSystem:"sp66" }; }
  function normalizeMonster(m) {
    ...
    const mappedAbility = canonicalAbilityId(ABILITIES.find(a => a.name === m.ability)?.id || "clearBody");
    const mappedMoves = (m.moves || []).map(old => canonicalMoveId(MOVES.find(move => move.name === old.name)?.id)).filter(Boolean);
    return { id:m.id||uid(), name:m.name||"이름 없음", image:m.image||"", level:clamp(m.level||50,1,100), type1:m.type1||"노말", type2:m.type2||"", baseStats, ivs,
      allowedAbilityIds:(Array.isArray(m.allowedAbilityIds)&&m.allowedAbilityIds.length ? [...new Set(m.allowedAbilityIds.map(canonicalAbilityId))] : [mappedAbility]),
      allowedMoveIds:(Array.isArray(m.allowedMoveIds)&&m.allowedMoveIds.length ? [...new Set(m.allowedMoveIds.map(canonicalMoveId))] : (mappedMoves.length?mappedMoves:["tackle"])) };
  }
  function normalizeState(raw) {
    ...
    result.team = (Array.isArray(raw?.team)?raw.team:[]).map(item => {
      ...
      const validMoves=(item.moveIds||[]).map(canonicalMoveId).filter(id=>species.allowedMoveIds.includes(id)).slice(0,4),abilityId=canonicalAbilityId(item.abilityId);
      return { ...defaultBuild(species), ...item, trainingSystem:"sp66", evs:normalizeStatPoints(item.evs,item.trainingSystem), moveIds:validMoves.length?validMoves:species.allowedMoveIds.slice(0,4), abilityId:species.allowedAbilityIds.includes(abilityId)?abilityId:species.allowedAbilityIds[0] };
  ...
  function movePowerLabel(move){if(move.category==="status")return "—";if(move.variablePower)return "변동";if(move.ohko)return "일격";if(move.fixedDamage)return "고정";return Number(move.power)||"—";}
  function moveAccuracyLabel(move){return move.alwaysHit||Number(move.accuracy)>=1000?"—":Number(move.accuracy)||"—";}
  ...
  function getAbility(id){return ABILITY_BY_ID[id]||ABILITY_BY_ID[dexId(id)]||ABILITIES[0];}
  function computeStats(species,evs={}) { const level=species.level,points=normalizeStatPoints(evs,"sp66"),out={};for(const [key] of STATS){const base=Number(species.baseStats[key]),iv=Number(species.ivs[key]),natural=key==="hp"?Math.floor(((2*base+iv)*level)/100)+level+10:Math.floor(((2*base+iv)*level)/100)+5;out[key]=natural+points[key];}return out; }
  ...
  function renderHome(){const count=state.monsters.length,team=state.team.length;els.content.innerHTML=`<div class="view home-view"><div class="arena-glow"></div><div class="home-content"><div class="home-copy"><span class="eyebrow">BUILD · COMMAND · CONQUER</span><h1>ISHS <span>ARENA</span></h1><p>도감에 나만의 몬스터를 등록하고 실제 기술·특성으로 배틀하세요. 개체값과 능력 포인트가 전략을 완성합니다.</p><div class="home-actions"><button class="primary-button" data-action="${count?"go-team":"add-monster"}"><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7L8 5Z"/></svg>${count?"팀 만들기":"첫 몬스터 등록"}</button><button class="secondary-button" data-action="go-roster">몬스터 도감</button></div><div class="home-meta"><div><b>${String(count).padStart(2,"0")}</b><small>DEX</small></div><div><b>${String(team).padStart(2,"0")}/06</b><small>BATTLE TEAM</small></div><div><b>${count&&team?"READY":"SETUP"}</b><small>STATUS</small></div></div></div><div class="battle-orb-stage" aria-hidden="true"><div class="battle-orb"><span class="orb-line"></span><span class="orbit"><i></i></span></div><div class="stage-label"><small>COMPETITIVE CORE</small><b>IV · SP 66 시스템 온라인</b></div></div></div></div>`;}
  ...
  function renderMoveLibrary(query=""){
    ...
    els.moveLibrary.innerHTML=shown.map(m=>`<label class="library-choice move-choice"><input type="checkbox" data-library-move value="${m.id}" ${draftMoves.has(m.id)?"checked":""}><span class="choice-card"><i class="choice-check">✓</i><span class="choice-info"><b>${esc(m.name)}${m.englishName&&m.englishName!==m.name?` <i class="english-name">${esc(m.englishName)}</i>`:""}</b><small>${esc(m.description)}</small></span><span class="move-meta"><i class="coverage-badge ${m.engineCoverage||"verified"}">${m.engineCoverage==="partial"?"고유":m.engineCoverage==="verified"?"정밀":"자동"}</i><i class="move-category ${m.category}">${CAT[m.category]}</i><i class="move-type-mini" style="--mini-color:${TYPES[m.type]?.[0]||TYPES["노말"][0]}">${m.type} · 위력 ${movePowerLabel(m)} · 명중 ${moveAccuracyLabel(m)} · PP ${defaultMovePp(m)}</i></span></span></label>`).join("")+(matches.length>shown.length?`<p class="library-more">${matches.length-shown.length}개가 더 있습니다. 이름을 검색하면 모두 찾을 수 있습니다.</p>`:"");
  ...
  function openBuild(species,index=-1){if(index<0&&state.team.length>=6)return toast("팀은 최대 6마리까지 편성할 수 있습니다.","error");const existing=index>=0?state.team[index]:defaultBuild(species);els.buildForm.elements.speciesId.value=species.id;els.buildForm.elements.buildIndex.value=index;$("#buildModalTitle").textContent=`${species.name} 배틀 세팅`;$("#buildSummary").style.cssText=typeStyle(species.type1);$("#buildSummary").innerHTML=`<div class="build-summary-image">${imageMarkup(species)}</div><div class="build-summary-info"><h3>${esc(species.name)} <small>Lv.${species.level}</small></h3><div class="type-tags">${typeTag(species.type1)}${typeTag(species.type2)}</div><p>IV ${STATS.map(([k])=>species.ivs[k]).join(" / ")} · 허용 기술 ${species.allowedMoveIds.length}개 / 잠김 ${MOVES.length-species.allowedMoveIds.length}개 · 허용 특성 ${species.allowedAbilityIds.length}개</p></div>`;$("#buildAbilityList").innerHTML=species.allowedAbilityIds.map((id,i)=>{const a=getAbility(id);return`<label class="radio-choice"><input type="radio" name="buildAbility" value="${id}" ${existing.abilityId===id||(!existing.abilityId&&i===0)?"checked":""}><span><b>${esc(a.name)}</b><small>${esc(a.description)}</small></span></label>`}).join("");draftBuildMoves=new Set(existing.moveIds||[]);renderBuildMoves(species);$("#evInputs").innerHTML=STATS.map(([k,l])=>`<label class="ev-field"><span>${l}</span><input name="ev_${k}" data-ev-key="${k}" type="number" min="0" max="${STAT_POINT_PER_STAT}" step="1" value="${clamp(existing.evs?.[k],0,STAT_POINT_PER_STAT)}"><i class="ev-bar"><i data-ev-bar="${k}"></i></i></label>`).join("");updateEvPreview(species);els.buildModal.hidden=false;document.body.style.overflow="hidden";}
  function renderBuildMoves(species){$("#buildMoveList").innerHTML=species.allowedMoveIds.map(id=>{const m=MOVE_BY_ID[id]||MOVE_BY_ID[dexId(id)];if(!m)return"";const checked=draftBuildMoves.has(id),disabled=!checked&&draftBuildMoves.size>=4;return`<label class="build-move-choice"><input type="checkbox" data-build-move value="${id}" ${checked?"checked":""} ${disabled?"disabled":""}><span style="border-left:4px solid ${TYPES[m.type]?.[0]||TYPES["노말"][0]}"><b>${esc(m.name)} · ${m.type} · ${CAT[m.category]} · 위력 ${movePowerLabel(m)} · 명중 ${moveAccuracyLabel(m)} · PP ${defaultMovePp(m)}</b><small>${esc(m.description)}</small></span></label>`}).join("");$("#buildMoveCount").textContent=`${draftBuildMoves.size} / 4`;}
  function readEvs(){return Object.fromEntries(STATS.map(([k])=>[k,clamp(els.buildForm.elements[`ev_${k}`]?.value,0,STAT_POINT_PER_STAT)]));}
  function updateEvPreview(species){const evs=readEvs(),total=Object.values(evs).reduce((a,b)=>a+b,0),stats=computeStats(species,evs);$("#evTotal").textContent=total;$("#evTotal").parentElement.classList.toggle("over",total>STAT_POINT_TOTAL);STATS.forEach(([k])=>{const bar=$(`[data-ev-bar="${k}"]`);if(bar)bar.style.width=`${evs[k]/STAT_POINT_PER_STAT*100}%`;});$("#computedStats").innerHTML=STATS.map(([k,l])=>`<div class="computed-stat"><b>${stats[k]}</b><small>실제 ${l} · SP +${evs[k]}</small></div>`).join("");}
  function closeBuild(){els.buildModal.hidden=true;document.body.style.overflow="";}
  function submitBuild(e){e.preventDefault();const species=getSpecies(els.buildForm.elements.speciesId.value),index=Number(els.buildForm.elements.buildIndex.value),evs=readEvs(),total=Object.values(evs).reduce((a,b)=>a+b,0),abilityId=new FormData(els.buildForm).get("buildAbility");if(total>STAT_POINT_TOTAL)return toast(`능력 포인트 합계는 ${STAT_POINT_TOTAL}을 넘을 수 없습니다.`,"error");if(!draftBuildMoves.size)return toast("기술을 최소 1개 선택하세요.","error");const build={speciesId:species.id,abilityId,moveIds:[...draftBuildMoves],evs,trainingSystem:"sp66"};if(index>=0)state.team[index]=build;else state.team.push(build);saveState("build");closeBuild();renderTeam();toast(`${species.name}의 배틀 세팅을 적용했습니다.`);}

  function renderBattleSetup(){const count=state.team.length,need=selectedFormat==="double"?2:1,ready=count>=need;els.content.innerHTML=`<div class="view battle-setup"><div class="view-inner"><div class="page-heading"><div><span class="eyebrow">BATTLE GATE</span><h1>배틀 형식 선택</h1><p>개체값과 능력 포인트, 선택한 실제 기술·특성이 모두 전투 계산에 반영됩니다.</p></div></div><div class="format-selector"><button class="format-card ${selectedFormat==="single"?"selected":""}" data-action="select-format" data-format="single"><span class="format-number">1</span><span class="format-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M5 21a7 7 0 0 1 14 0"/></svg></span><h2>싱글 배틀</h2><p>한 번에 한 마리가 필드에 나섭니다. 팀의 선두 3마리까지 참가합니다.</p><small>TEAM SIZE · UP TO 3</small></button><button class="format-card ${selectedFormat==="double"?"selected":""}" data-action="select-format" data-format="double"><span class="format-number">2</span><span class="format-icon"><svg viewBox="0 0 24 24"><circle cx="8" cy="8" r="3"/><circle cx="17" cy="9" r="3"/><path d="M2 20a6 6 0 0 1 12 0m0 0a5 5 0 0 1 10 0"/></svg></span><h2>더블 배틀</h2><p>두 마리가 함께 싸우며 우선도와 특성 발동 순서가 중요합니다.</p><small>TEAM SIZE · UP TO 4</small></button></div><div class="setup-footer"><div class="team-ready"><span class="team-ready-indicator">${ready?"✓":"!"}</span><span><b>${ready?"팀 준비 완료":`몬스터 ${need}마리 필요`}</b><small>현재 팀 ${count}마리 · ${selectedFormat==="double"?"더블":"싱글"} 배틀</small></span></div><button class="primary-button" data-action="start-battle" ${ready?"":"disabled"}><svg viewBox="0 0 24 24"><path d="M8 5v14l11-7L8 5Z"/></svg>매치 시작</button></div></div></div>`;}
  ...
  function createCombatant(build,enemy=false){const species=getSpecies(build.speciesId),stats=computeStats(species,build.evs);return{...structuredClone(species),name:enemy?`섀도 ${species.name}`:species.name,stats,maxHp:stats.hp,currentHp:stats.hp,abilityId:build.abilityId,moves:build.moveIds.map(id=>MOVE_BY_ID[id]).filter(Boolean).map(raw=>{const move=structuredClone(raw),maxPp=defaultMovePp(move);return{...move,pp:maxPp,maxPp};}),stages:{attack:0,defense:0,spAttack:0,spDefense:0,speed:0,accuracy:0,evasion:0},status:null,toxicCounter:0,sleepTurns:0,protected:false,flashFireBoost:false,fainted:false,entered:false,flinched:false,hasActed:false,enemy};}
  function startBattle(){const size=selectedFormat==="double"?4:3,builds=state.team.slice(0,size);if(builds.length<(selectedFormat==="double"?2:1))return toast("팀 인원이 부족합니다.","error");battle={format:selectedFormat,playerTeam:builds.map(b=>createCombatant(b)),enemyTeam:[...builds].reverse().map(b=>createCombatant(b,true)),playerActive:[],enemyActive:[],hazards:{player:{stealthRock:false},enemy:{stealthRock:false}},turn:1,queued:[],actorCursor:0,phase:"select",messages:[]};battlePanel="moves";refreshActive();applyEntryAbilities();battle.messages.push(`상대는 ${battle.enemyTeam[battle.enemyActive[0]].name}을 내보냈다!`);renderBattle();beep(310,.08);}
  function resetSwitchState(monster){if(!monster)return;for(const [key] of BATTLE_STAGE_STATS)monster.stages[key]=0;monster.protected=false;monster.flinched=false;monster.hasActed=false;monster.toxicCounter=0;monster.entered=false;}
  function applyEntryHazards(side,index){const team=side==="player"?battle.playerTeam:battle.enemyTeam,monster=team[index];if(!monster||monster.fainted||!battle.hazards?.[side]?.stealthRock||abilityKey(monster)==="magicguard")return false;const effectiveness=effectValue({type:"바위"},monster),damage=Math.max(1,Math.floor(monster.maxHp*effectiveness/8));monster.currentHp=Math.max(0,monster.currentHp-damage);updateHp(side,index,monster);battle.messages.push(`${monster.name}에게 뾰족한 바위가 박혔다! (${damage} 데미지)`);if(monster.currentHp<=0){monster.fainted=true;battle.messages.push(`${monster.name}은 쓰러졌다!`);}return true;}
  function refreshActive(withEntryHazards=false){const count=battle.format==="double"?2:1;for(const side of ["player","enemy"]){const team=side==="player"?battle.playerTeam:battle.enemyTeam,key=side==="player"?"playerActive":"enemyActive",previous=[...(battle[key]||[])],kept=previous.filter(i=>!team[i].fainted&&team[i].currentHp>0).slice(0,count);for(let index=0;index<team.length&&kept.length<count;index++){const monster=team[index];if(kept.includes(index)||monster.fainted||monster.currentHp<=0)continue;kept.push(index);if(withEntryHazards&&!previous.includes(index)){resetSwitchState(monster);applyEntryHazards(side,index);if(monster.fainted)kept.pop();}}battle[key]=kept;}battle.actorCursor=Math.min(battle.actorCursor,Math.max(0,battle.playerActive.length-1));}
  function stageMultiplier(stage){const value=clamp(Number(stage)||0,-6,6);return value>=0?(2+value)/2:2/(2-value);}
  function accuracyMultiplier(stage){const value=clamp(Number(stage)||0,-6,6);return value>=0?(3+value)/3:3/(3-value);}
  ...
  function effectivenessBadge(move,target){if(!target||move.category==="status")return"";const value=effectValue(move,target),kind=value===0?"immune":value>1?"super":value<1?"resist":"normal",label=value===0?"효과 없음":value>1?"효과가 굉장함":value<1?"효과가 별로임":"보통 효과";return`<small class="move-matchup ${kind}">${label} · ×${value}</small>`;}
  function cpuMovePanel(actor){const hasPp=actor?.moves.some(move=>move.pp>0),target=battle?.enemyTeam?.[battle.enemyActive?.[0]];if(!hasPp)return`<div class="move-buttons"><button class="move-button" data-action="battle-move" data-move="-1" style="--move-color:${TYPES["노말"][0]}"><span class="move-type-label">노말</span><b>발버둥</b><small>PP가 모두 소진되어 이 기술만 사용할 수 있습니다.</small><span class="pp-count">PP —</span></button></div>`;return`<div class="move-buttons">${actor.moves.map((m,i)=>`<button class="move-button" data-action="battle-move" data-move="${i}" style="--move-color:${TYPES[m.type]?.[0]||TYPES["노말"][0]};--move-tint:${TYPES[m.type]?.[0]||TYPES["노말"][0]}33" ${battleBusy||m.pp<=0?"disabled":""}><span class="move-type-label">${esc(m.type)}</span><b>${esc(m.name)}</b><small>${CAT[m.category]} · 위력 ${movePowerLabel(m)} · 명중 ${moveAccuracyLabel(m)}</small><span class="pp-count">PP <b>${m.pp}</b> / ${m.maxPp}</span>${effectivenessBadge(m,target)}<small class="move-effect-line">${esc(m.description)}</small></button>`).join("")}</div>`;}
  ...
  function cpuStatusPanel(){const groups=[["내 포켓몬","player",battle.playerTeam,battle.playerActive],["상대 포켓몬","enemy",battle.enemyTeam,battle.enemyActive]];return`<div class="battle-status-panel">${groups.map(([label,side,team,active])=>`<section><div class="status-panel-title"><b>${label}</b><span>${battle.hazards?.[side]?.stealthRock?"스텔스록 설치됨":"장판 없음"}</span></div>${active.map(i=>{const m=team[i];return`<article><div><b>${esc(m.name)}</b><small>${m.status?STATUS_LABEL[m.status]:"정상"} · 특성 ${esc(abilityName(m))}</small></div><div class="stage-grid">${BATTLE_STAGE_STATS.map(([k,l])=>{const stage=m.stages[k]||0,multiplier=["accuracy","evasion"].includes(k)?accuracyMultiplier(stage):stageMultiplier(stage),detail=["accuracy","evasion"].includes(k)?`배율 ×${Number(multiplier.toFixed(2))}`:`현재 ${Math.floor(effectiveStat(m,k))}`;return`<span class="stage-cell ${stage>0?"up":stage<0?"down":""}"><small>${l}</small><b>${stage>0?`+${stage}`:stage}</b><i>${detail}</i></span>`;}).join("")}</div></article>`;}).join("")}</section>`).join("")}</div>`;}
  ...
  function setLog(text){const l=$("#battleLog");if(!l)return;l.textContent=text;l.classList.remove("super","resist","immune","critical");const kind=/굉장/.test(text)?"super":/별로/.test(text)?"resist":/효과가 없|통하지/.test(text)?"immune":/급소/.test(text)?"critical":"";if(kind)l.classList.add(kind);void l.offsetWidth;l.classList.add("message-pop");setTimeout(()=>l.classList.remove("message-pop"),360);}
  ...
  async function executeAction(action){const own=action.side==="player"?battle.playerTeam:battle.enemyTeam,foes=action.side==="player"?battle.enemyTeam:battle.playerTeam,active=action.side==="player"?battle.enemyActive:battle.playerActive,ownActive=action.side==="player"?battle.playerActive:battle.enemyActive,actor=own[action.actorIndex];if(action.kind==="switch"){const position=ownActive.indexOf(action.actorIndex),next=own[action.switchIndex];if(position<0||!next||next.fainted||ownActive.includes(action.switchIndex))return;setLog(`${actor.name}, 돌아와!`);resetSwitchState(actor);await delay(260);ownActive[position]=action.switchIndex;resetSwitchState(next);setLog(`${next.name}! 너로 정했다!`);applyEntryHazards(action.side,action.switchIndex);applyEntryAbilities();await delay(480);return;}if(!actor||actor.fainted)return;if(!await preAction(actor)){actor.hasActed=true;await delay(650);return;}const targets=active.filter(i=>!foes[i].fainted);if(!targets.length)return;const targetIndex=targets[Math.floor(Math.random()*targets.length)],target=foes[targetIndex],targetSide=action.side==="player"?"enemy":"player",move=action.move;if(move.id!=="struggle"){if(move.pp<=0)return;move.pp=Math.max(0,move.pp-1);}setLog(`${actor.name}의 ${move.name}!`);animate(action.side,action.actorIndex,"attack");beep(action.side==="player"?420:260,.07);await delay(430);
    ...
    if(move.category==="status"&&move.selfStages){applyStages(actor,move.selfStages);setLog(battle.messages.at(-1)||`${actor.name}의 능력치가 변했다!`);actor.hasActed=true;await delay(600);return;}
    if(move.sideCondition==="stealthrock"){if(battle.hazards[targetSide].stealthRock)setLog("하지만 이미 스텔스록이 설치되어 있다!");else{battle.hazards[targetSide].stealthRock=true;setLog(`${targetSide==="enemy"?"상대":"우리"} 필드 주위에 뾰족한 바위가 떠올랐다!`);}actor.hasActed=true;await delay(650);return;}
    if(target.protected){setLog(`${target.name}은 공격을 방어했다!`);actor.hasActed=true;await delay(600);return;}
    const alwaysHit=move.alwaysHit||abilityKey(actor)==="noguard"||abilityKey(target)==="noguard";let accuracy=move.id==="toxic"&&(actor.type1==="독"||actor.type2==="독")?100:move.accuracy;accuracy*=accuracyMultiplier((actor.stages.accuracy||0)-(target.stages.evasion||0));if(abilityKey(actor)==="compoundeyes")accuracy*=1.3;if(abilityKey(actor)==="victorystar")accuracy*=1.1;if(abilityKey(actor)==="hustle"&&move.category==="physical")accuracy*=.8;if(!alwaysHit&&Math.random()*100>accuracy){setLog("공격이 빗나갔다!");actor.hasActed=true;await delay(600);return;}
    if(abilityImmunity(actor,target,move)){updateHp(targetSide,targetIndex,target);actor.hasActed=true;await delay(700);return;}
    if(move.forceSwitch){await forceRandomSwitch(targetSide,targetIndex);actor.hasActed=true;return;}
    ...
    if(move.category==="status"&&move.targetStages){applyStages(target,move.targetStages,true);setLog(battle.messages.at(-1)||`${target.name}의 능력치가 변했다!`);actor.hasActed=true;await delay(650);return;}
    const result=damageResult(actor,target,move);let hits=1;if(Array.isArray(move.multihit)){const[min,max]=move.multihit;hits=min+Math.floor(Math.random()*(max-min+1));}else if(Number(move.multihit)>1)hits=Number(move.multihit);if(result.type!==0){if(move.ohko)result.damage=target.currentHp;else if(move.fixedDamage==="level")result.damage=actor.level;else if(Number(move.fixedDamage))result.damage=Number(move.fixedDamage);else result.damage*=hits;}target.currentHp=Math.max(0,target.currentHp-result.damage);updateHp(targetSide,targetIndex,target);animate(targetSide,targetIndex,"hit");beep(130,.1);
    const notices=[];if(result.type===0)notices.push("효과가 없다…");else if(result.type>1)notices.push("효과가 굉장했다!");else if(result.type<1)notices.push("효과가 별로인 듯하다…");if(result.critical)notices.push("급소에 맞았다!");if(hits>1)notices.push(`${hits}번 맞았다!`);if(notices.length)setLog(notices.join(" "));await delay(result.type>1?1000:700);
  ...
  window.ISHSArena={hydrate:hydrateFromServer,getTeamSnapshot,currentRoute:()=>route,navigate,toast,getState:()=>structuredClone(state),typeEffect:(move,target)=>effectValue(move,target)};
