(() => {
  ...
  const FULL_DEX_CACHE_KEY = "nexus-full-dex-cache-v2";
  const LEGACY_FULL_DEX_CACHE_KEY = "nexus-full-dex-cache-v1";
  ...
  function koreanMoveDescription(data) {
    ...
    if(data.basePowerCallback||data.condition||data.weather||data.terrain||data.sideCondition||data.slotCondition||data.selfSwitch||data.forceSwitch||data.callsMove||data.twoTurnMove)parts.push("전투 조건에 따라 추가 효과가 달라질 수 있다");
  ...
  function loadCachedFullDex() {
    try { const cache=JSON.parse(localStorage.getItem(FULL_DEX_CACHE_KEY));if(cache?.version===2&&Array.isArray(cache.moves)&&Array.isArray(cache.abilities)){mergeFullDex(cache.moves,cache.abilities);fullDexState={status:"ready",moves:MOVES.length,abilities:ABILITIES.length,source:"저장된 전체 데이터"};return true;} } catch {}
    return false;
  }
  async function loadFullDex() {
    ...
    try {
      ...
      try{localStorage.removeItem(LEGACY_FULL_DEX_CACHE_KEY);localStorage.setItem(FULL_DEX_CACHE_KEY,JSON.stringify({version:2,moves:convertedMoves,abilities:convertedAbilities}));}catch{}
