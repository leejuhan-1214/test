(() => {
  ...
  const STATS=[["attack","공격"],["defense","방어"],["spAttack","특공"],["spDefense","특방"],["speed","스피드"],["accuracy","명중률"],["evasion","회피율"]];
  ...
  function stageMultiplier(key,stage){const value=Math.max(-6,Math.min(6,Number(stage)||0));return ["accuracy","evasion"].includes(key)?(value>=0?(3+value)/3:3/(3-value)):(value>=0?(2+value)/2:2/(2-value));}
  ...
  function matchupBadge(move,target){if(!target||move.category==="status")return"";const value=arena()?.typeEffect?.(move,target)??1,kind=value===0?"immune":value>1?"super":value<1?"resist":"normal",label=value===0?"효과 없음":value>1?"효과가 굉장함":value<1?"효과가 별로임":"보통 효과";return`<small class="move-matchup ${kind}">${label} · ×${value}</small>`;}
  function statusPanel(battle,ownSide){return `<div class="online-status-panel">${[ownSide,1-ownSide].map((side,row)=>{const monster=battle.teams[side][battle.active[side]],hazard=battle.hazards?.[side]?.stealthRock?" · 스텔스록 설치됨":"";return `<section><div class="status-panel-title"><b>${row===0?"내":"상대"} ${esc(monster.name)}</b><span>${monster.status?STATUS[monster.status]:"정상"}${hazard}</span></div><div class="stage-grid">${STATS.map(([key,label])=>{const stage=monster.stages[key]||0,multiplier=stageMultiplier(key,stage),detail=["accuracy","evasion"].includes(key)?`배율 ×${Number(multiplier.toFixed(2))}`:`현재 ${Math.floor((monster.stats[key]||1)*multiplier)}`;return`<div class="stage-cell ${stage>0?"up":stage<0?"down":""}"><small>${label}</small><b>${stageText(stage)}</b><span>${detail}</span></div>`;}).join("")}</div><p>특성 <b>${esc(monster.ability?.name||"—")}</b></p></section>`;}).join("")}</div>`;}
  ...
  function renderOnlineBattle(host,room){
    ...
    host.innerHTML=`<div class="view battle-view online-battle"><div class="stadium"><div class="stadium-lights"></div></div><div class="battle-hud"><div class="battle-topline"><span class="battle-mode-label"><i class="live-dot"></i> PRIVATE ROOM · ${room.code}</span><div class="turn-timer">${battle.turn}</div></div>
      ...
      <div class="battle-log online-log ${esc(last?.kind||"")}"><small>${esc(battle.nicknames[foeSide])} VS ${esc(battle.nicknames[ownSide])}</small><b>${esc(last?.text||"배틀을 준비하고 있습니다.")}</b></div>
      ${recentEffect?`<div class="battle-effect-banner ${esc(recentEffect.kind)}">${esc(recentEffect.text)}</div>`:""}
